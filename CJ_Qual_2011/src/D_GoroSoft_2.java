/*
 * To change this D_GoroSoft_2, choose Tools | D_GoroSoft_2s
 * and open the D_GoroSoft_2 in the editor.
 */
...
public class D_GoroSoft_2 {
    @SuppressWarnings("CallToThreadDumpStack")
    public static void main(String[] args) {
        try {
            D_GoroSoft_2 solver = new D_GoroSoft_2();
            solver.openIO();
            solver.solve();
            solver.closeIO();
        }
    ...
    final static String FILENAME = "D-small-attempt2";
...
/******************************************************************************/
    void solve() throws IOException {
        int tc = sc.nextInt();
        int size, i, targetPos, curMoveCount;
        double moveCount;
        int[] initList;
        ...
        for(int tci = 1; tci <= tc; tci++) {
            ...
            for(i = 0; i < size; i++) {
                ...
                //System.out.println("curMoveCount: " + curMoveCount);

                if(curMoveCount != 0) {
                    curMoveCount++;
                    double fi = 1;
                    while(curMoveCount >= 1) {
                        fi *= curMoveCount;
                        curMoveCount--;
                    }
                    moveCount += fi;
