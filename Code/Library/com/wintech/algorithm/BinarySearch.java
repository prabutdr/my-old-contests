...
public class BinarySearch {
     ...
     * @return
     */
    public static int rankFirst(int key, int[] array) {
        int li = 0;
        int hi = array.length - 1;
        int rank = 0;
        while(li <= hi) {
    ...
    /**
     * Find index/rank of key in array[], also giving count of distinct values
     *
     * @param key
     * @param array - should be sorted
     * @return  rank of a key if exist
     *          -1 otherwise
     */
   /* public static int rankByDistinctCount(int key, int[] array, int li, int hi, int dcount) {
        if(dcount == 0 || li > hi) return -1;
        ...
        if(array[mid] < key) {
            idx = mid + 1;
            while(idx < array.length && array[idx] == array[mid]) idx++;
            return rankByDistinctCount(key, array, idx, hi, dcount);
        }
        if(array[mid] > key) {
            idx = mid - 1;
            while(idx >= 0 && array[idx] == array[mid]) idx--;
            return rankByDistinctCount(key, array, li, idx, dcount);
        }
...
//        out.close();
        ...
        System.out.println("Rank first: " + rankFirst(4, array));
        /*StdOut.println();
        for(int i = 0; i < array.length; i++) {
            StdOut.printf("rankByDistinctCount(%d) : %d\n", array[i],
                    rankByDistinctCount(array[i], array, 0, array.length-1, 5));
        } */
    }
