package on2012_8_10.ch2_erdosnumbers;
...
public class Ch2_ErdosNumbers {
	public void solve(int testNumber, BaseIn in, PrintWriter out) {
        ...
        HashMap<String, Integer> authorsRankFinal = new HashMap<String, Integer>();
        while(updateReq) {
            updateReq = false;
                    ...
                    for(int i = 0; i < coauthors.size(); i++) {
                        ...
                        if(list.contains(entry.getKey())) {
                            for(String author: list) {
                                if(!authorsRankFinal.containsKey(author) && !authorsRank.containsKey(author)) {
            ...
            authorsRankFinal.putAll(authorsRank);
            authorsRank.clear();
        ...
        for(int i = 0; i < n; i++) {
            line = in.readLine();
            rank = authorsRankFinal.get(line);
            if(rank == null) {
